...
const DefaultData = new mongoose.model("DefaultData", postSchema);
...
DefaultData.find({}, function(err, foundData) {
  if (!err) {
    if (foundData.length === 0) {
      DefaultData.insertMany(defaultPosts, function(err) {
        if (err) {
          console.log(err);
        } else {
...
app.get("/", function(req, res) {
  DefaultData.findOne({title: "Home"}, function(err, foundPost) {
    if (err) {
      console.log(err);
    } else {
      res.render("home", {startingContent: foundPost.content, posts: posts});
    }
  });
});

app.get("/about", function(req, res) {
  // console.log(req.body.params.path);
